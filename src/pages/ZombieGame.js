...
function drawBarrier(ctx,x,y,ratio){
  ...
  if(ratio<0.5){ctx.strokeStyle='rgba(0,0,0,0.5)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x+s*0.3,y+s*0.2);ctx.lineTo(x+s*0.5,y+s*0.6);ctx.stroke();}
      ...
      <div style={{padding:'12px 14px 4px'}}>
        {steps.map((s,i)=>(
          <div key={i} style={{display:'flex',gap:12,alignItems:'flex-start',background:'rgba(255,255,255,0.07)',border:'2px solid rgba(255,255,255,0.1)',borderRadius:12,padding:'10px 12px',marginBottom:8}}>
            ...
            <div><div style={{fontWeight:900,color:'#ffcc00',fontSize:12,marginBottom:2,textTransform:'uppercase'}}>{s.title}</div>
            <div style={{color:'#ccc',fontSize:12,lineHeight:1.5}}>{s.desc}</div></div>
          </div>
        ))}
      </div>
      <div style={{padding:'0 14px 14px'}}>
        <button onClick={onDone} style={{width:'100%',background:'linear-gradient(180deg,#44dd44,#22aa22)',border:'none',borderRadius:12,padding:'14px',color:'#fff',fontSize:16,fontWeight:900,cursor:'pointer',boxShadow:'0 5px 0 #116611',textTransform:'uppercase',letterSpacing:1,display:'block'}}>LET'S GO! →</button>
      </div>
    </div>
  );
}
      ...
      <div style={{padding:14}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:14}}>
          {AVATARS.map(av=>{
            ...
            return(
              <div key={av.id} onClick={()=>setSel(av.id)} style={{background:isSel?`${av.bg}44`:'rgba(255,255,255,0.05)',border:`3px solid ${isSel?av.color:'rgba(255,255,255,0.1)'}`,borderRadius:14,padding:'12px 6px',textAlign:'center',cursor:'pointer',transition:'all 0.15s'}}>
        ...
        <button onClick={()=>onSelect(selAv)} style={{width:'100%',background:`linear-gradient(180deg,${selAv.color},${selAv.bg})`,border:'none',borderRadius:12,padding:'14px',color:'#fff',fontSize:15,fontWeight:900,cursor:'pointer',boxShadow:`0 5px 0 ${selAv.bg}`,textTransform:'uppercase'}}>
          PLAY AS {selAv.name} →
        </button>
      </div>
    </div>
...
function GameEngine({isSolo,roomCode,myName,myAvatar,isHost,roomData,onGameOver,onBack}){
  ...
  // ALL game state lives in one ref — no async state issues
  const G=useRef({
    // map
  ...
  const doSpawnZombies=()=>{
    ...
    const pts=[[1,1],[g.cols-2,1],[1,g.rows-2],[g.cols-2,g.rows-2],
  ...
  const tick=()=>{
    ...
    if(Math.abs(jv.x)>0.05||Math.abs(jv.y)>0.05){
      ...
      const M=TILE*0.32; // hitbox margin
      // try X
      const nx=g.px+dx;
  ...
  const handleAttack=()=>{
    ...
    let hit=false;
    g.zombies.forEach(z=>{
      if(Math.hypot(z.x-g.px,z.y-g.py)<TILE*2.2){z.hp--;hit=true;}
    });
    g.zombies=g.zombies.filter(z=>z.hp>0);
    setZombieCount(g.zombies.filter(z=>z.hp>0).length);
  ...
  return(
    <div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',background:'#000',fontFamily:"'Arial Black',Arial,sans-serif",overflow:'hidden',position:'relative'}}>
      ...
      <div style={{background:isNight?'linear-gradient(90deg,#200000,#100010)':'linear-gradient(90deg,#003300,#001a00)',padding:'5px 8px',display:'flex',alignItems:'center',gap:6,borderBottom:`3px solid ${isNight?'#ff0000':'#44aa00'}`,flexShrink:0}}>
        <button onClick={onBack} style={{background:'#1a1a1a',border:'2px solid #444',borderRadius:7,padding:'3px 8px',color:'#aaa',fontSize:10,cursor:'pointer',fontWeight:900,textTransform:'uppercase'}}>✕ EXIT</button>
      ...
      {/* CONTROLS */}
      <div style={{flex:'0 0 130px',background:'linear-gradient(180deg,#111,#0a0a0a)',borderTop:'2px solid #222',position:'relative',flexShrink:0}}>
        {/* JOYSTICK */}
        ...
        <div style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
          <button onClick={handleAttack} style={{background:isNight?'linear-gradient(180deg,#ff3333,#aa0000)':'rgba(50,50,50,0.6)',border:`2px solid ${isNight?'#ff6666':'#333'}`,borderRadius:10,width:62,height:48,color:'#fff',fontSize:11,fontWeight:900,cursor:'pointer',opacity:isNight?1:0.4,textTransform:'uppercase',lineHeight:1.2}}>⚔️<br/>ATTACK</button>
          <button onClick={handleBuild} style={{background:!isNight?'linear-gradient(180deg,#886633,#553311)':'rgba(50,50,50,0.6)',border:`2px solid ${!isNight?'#ddaa55':'#333'}`,borderRadius:10,width:62,height:48,color:'#fff',fontSize:11,fontWeight:900,cursor:'pointer',opacity:!isNight?1:0.4,textTransform:'uppercase',lineHeight:1.2}}>🧱<br/>BUILD</button>
          <button onClick={handleRemove} style={{background:'linear-gradient(180deg,#663333,#441111)',border:'2px solid #cc4444',borderRadius:10,width:62,height:48,color:'#fff',fontSize:10,fontWeight:900,cursor:'pointer',textTransform:'uppercase',lineHeight:1.2,gridColumn:'1/-1'}}>🗑️ REMOVE</button>
        </div>
      </div>

      {/* DEATH */}
      {dead&&(
        <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.88)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:50}}>
          ...
          <button onClick={onBack} style={{background:'linear-gradient(180deg,#44dd44,#22aa22)',border:'none',borderRadius:12,padding:'14px 32px',color:'#fff',fontSize:16,fontWeight:900,cursor:'pointer',boxShadow:'0 5px 0 #116611',textTransform:'uppercase'}}>PLAY AGAIN</button>
        </div>
      )}
    </div>
  );
  ...
  return(
    <div style={{minHeight:'100%',background:'linear-gradient(180deg,#001a2e,#001a00)',fontFamily:"'Arial Black',Arial,sans-serif",overflowY:'auto'}}>
      ...
      <div style={{padding:16,textAlign:'center'}}>
        ...
        {isHost?<button onClick={onStart} style={{width:'100%',background:'linear-gradient(180deg,#44dd44,#22aa22)',border:'none',borderRadius:12,padding:14,color:'#fff',fontSize:15,fontWeight:900,cursor:'pointer',boxShadow:'0 5px 0 #116611',textTransform:'uppercase',marginBottom:10}}>▶ START GAME!</button>:<div style={{color:'#aaa',fontSize:13,marginBottom:10,padding:10,background:'rgba(255,255,255,0.05)',borderRadius:10}}>Waiting for host to start...</div>}
  ...
  return(
    <div style={{minHeight:'100%',background:'linear-gradient(180deg,#0a0010,#000a00)',fontFamily:"'Arial Black',Arial,sans-serif",overflowY:'auto'}}>
      ...
      <div style={{padding:14}}>
        ...
        <button onClick={()=>setScreen('howtoplay')} style={{width:'100%',background:'rgba(255,255,255,0.06)',border:'2px solid rgba(255,255,255,0.12)',borderRadius:10,padding:10,color:'#aaa',fontSize:13,cursor:'pointer',textTransform:'uppercase',fontWeight:900,marginBottom:8}}>📖 HOW TO PLAY</button>
