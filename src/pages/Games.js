...
function RacingGame({ playerName, onScore }) {
  ...
  const drawGame=useCallback(()=>{const canvas=canvasRef.current;if(!canvas)return;const ctx=canvas.getContext('2d');const st=stateRef.current;ctx.fillStyle='#2d7a2d';ctx.fillRect(0,0,W,H);ctx.fillStyle='#267026';for(let i=0;i<H;i+=12){ctx.fillRect(0,i,GRASS_W,6);ctx.fillRect(W-GRASS_W,i,GRASS_W,6);}ctx.fillStyle='#1a5c1a';ctx.fillRect(GRASS_W-4,0,4,H);ctx.fillRect(W-GRASS_W,0,4,H);ctx.fillStyle='#404040';ctx.fillRect(ROAD_X,0,ROAD_W,H);ctx.fillStyle='#383838';for(let i=0;i<H;i+=20){ctx.fillRect(ROAD_X,i,ROAD_W,10);}ctx.strokeStyle='#ffffff';ctx.lineWidth=2;ctx.setLineDash([25,18]);ctx.lineDashOffset=-st.lineOffset;for(let l=1;l<LANES;l++){ctx.strokeStyle=l===LANES/2?'#ffff00':'#ffffff';ctx.lineWidth=l===LANES/2?3:1.5;ctx.beginPath();ctx.moveTo(ROAD_X+l*LANE_W,0);ctx.lineTo(ROAD_X+l*LANE_W,H);ctx.stroke();}ctx.setLineDash([]);ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(ROAD_X,0);ctx.lineTo(ROAD_X,H);ctx.stroke();ctx.beginPath();ctx.moveTo(ROAD_X+ROAD_W,0);ctx.lineTo(ROAD_X+ROAD_W,H);ctx.stroke();st.traffic.forEach(t=>drawCar(ctx,t.x,t.y,t.color));(st.coffees||[]).forEach(c=>{const pulse=0.8+0.2*Math.sin(Date.now()/300+c.id);ctx.save();ctx.shadowColor='#ffcc00';ctx.shadowBlur=10*pulse;ctx.font='18px serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('☕',c.x,c.y);ctx.restore();ctx.strokeStyle=`rgba(255,200,0,${0.4*pulse})`;ctx.lineWidth=2;ctx.beginPath();ctx.arc(c.x,c.y,14*pulse,0,Math.PI*2);ctx.stroke();});st.car.x+=(st.targetX-st.car.x)*0.18;drawCar(ctx,st.car.x,st.car.y,'#ff3333',true);ctx.fillStyle='rgba(0,0,0,0.45)';ctx.fillRect(ROAD_X,0,70,36);ctx.fillStyle='#ffff00';ctx.font='bold 13px Arial';ctx.textAlign='left';ctx.fillText(`${Math.floor(80+st.speed*20)}km/h`,ROAD_X+6,22);},[W,H,GRASS_W,ROAD_X,ROAD_W,LANES,LANE_W]);
...
function BaristaLeft({ state }) {
  ...
  return (
    <svg width="72" height="110" viewBox="0 0 72 110" style={{ overflow: 'visible' }}>
      ...
      {eyeStyle === 'happy' && <><ellipse cx="30" cy="32" rx="2.5" ry="2.5" fill="#3a2010"/><ellipse cx="42" cy="32" rx="2.5" ry="2.5" fill="#3a2010"/></>}
      {eyeStyle === 'angry' && <><line x1="27" y1="29" x2="33" y2="31" stroke="#3a2010" strokeWidth="2"/><line x1="39" y1="31" x2="45" y2="29" stroke="#3a2010" strokeWidth="2"/><ellipse cx="30" cy="33" rx="2" ry="2" fill="#3a2010"/><ellipse cx="42" cy="33" rx="2" ry="2" fill="#3a2010"/></>}
      {eyeStyle === 'sad' && <><line x1="27" y1="31" x2="33" y2="29" stroke="#3a2010" strokeWidth="1.5"/><line x1="39" y1="29" x2="45" y2="31" stroke="#3a2010" strokeWidth="1.5"/><ellipse cx="30" cy="33" rx="2" ry="2" fill="#3a2010"/><ellipse cx="42" cy="33" rx="2" ry="2" fill="#3a2010"/></>}
      ...
      <g transform={`rotate(${armLAngle}, 22, 52)`}>
        ...
        {state === 'wave' && <><ellipse cx="4" cy="50" rx="3" ry="4" fill="#e8b89a"/><ellipse cx="1" cy="48" rx="2" ry="3" fill="#e8b89a"/></>}
        {state === 'fight' && <rect x="2" y="51" width="8" height="6" rx="3" fill="#e8b89a"/>}
      </g>
      <g transform={`rotate(${-armRAngle}, 50, 52)`}>
        ...
        {state === 'cheer' && <text x="66" y="46" fontSize="12">☕</text>}
      </g>
      ...
      {state === 'wave' && <animateTransform attributeName="transform" type="rotate" values="0 36 54;3 36 54;-3 36 54;0 36 54" dur="0.8s" repeatCount="indefinite"/>}
      {state === 'cheer' && <animateTransform attributeName="transform" type="translate" values="0 0;0 -4;0 0" dur="0.5s" repeatCount="indefinite"/>}
      {state === 'fight' && <animateTransform attributeName="transform" type="translate" values="0 0;3 0;0 0" dur="0.3s" repeatCount="indefinite"/>}
      {state === 'sad' && <animateTransform attributeName="transform" type="translate" values="0 0;0 2;0 0" dur="1.2s" repeatCount="indefinite"/>}
    </svg>
  );
}
  ...
  return (
    <svg width="72" height="110" viewBox="0 0 72 110" style={{ overflow: 'visible', transform: 'scaleX(-1)' }}>
      ...
      <g transform={`rotate(${armLAngle}, 22, 52)`}>
        ...
        {state === 'wave' && <><ellipse cx="4" cy="50" rx="3" ry="4" fill="#d4a080"/><ellipse cx="1" cy="48" rx="2" ry="3" fill="#d4a080"/></>}
    ...
    <div style={{ height: '100%', background: '#121213', color: '#fff', display: 'flex', flexDirection: 'column', fontFamily: "'Arial',sans-serif", overflow: 'hidden' }}>
      ...
      {gameState !== 'playing' && (
        <div style={{ textAlign: 'center', padding: '8px 14px', background: gameState === 'won' ? '#538d4e22' : '#ff444422', borderTop: `1px solid ${gameState === 'won' ? '#538d4e' : '#ff4444'}`, flexShrink: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 'bold', color: gameState === 'won' ? '#538d4e' : '#ff6b6b', marginBottom: 6 }}>{gameState === 'won' ? `🎉 ${guesses.length <= 2 ? 'Genius!' : guesses.length <= 4 ? 'Great job!' : 'Got it!'}` : `😔 It was "${word}"`}</div>
